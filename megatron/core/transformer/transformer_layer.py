...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def _callable_wrapper(self, is_forward, func, stream, event, *inputs, nvtx_name=None):
        """
        ...
        if is_forward:
            ...
            with torch.cuda.stream(stream):
                ...
                detached_inputs = [make_viewless_tensor(inp=e, requires_grad=e.requires_grad, keep_graph=True).detach() if isinstance(e, Tensor) else e for e in inputs]
                for i, input_tensor in enumerate(detached_inputs):
                    if isinstance(input_tensor, Tensor):
                        input_tensor.requires_grad = inputs[i].requires_grad
                outputs = func(*detached_inputs)
                if not isinstance(outputs, tuple):
                    outputs = (outputs,)
                outputs = tuple([make_viewless_tensor(inp=e, requires_grad=e.requires_grad, keep_graph=True) if isinstance(e, Tensor) else e for e in outputs])
            event.record(stream)
            torch.cuda.nvtx.range_pop()

            return outputs, detached_inputs
        else:
            event.wait(stream)
            with torch.cuda.stream(stream):
